...
### Q-006
**Data:** 2026-08-14
**Tarefa:** DV2-DEV-003 (DV2-SPRINT-001, Etapa 5)
**Requisito:** docs/ARCHITECTURE.md — Dependências
**Pergunta:** `docs/ARCHITECTURE.md` lista explicitamente as dependências `Web -> Application`, `Infrastructure -> Application`, `Infrastructure -> Domain`, `Application -> Domain`, mas **não lista `Web -> Infrastructure`**. A fundação de persistência (DV2-DEV-003) exigiu que `DocsViewer.Web` (`Program.cs`, composition root) referenciasse `DocsViewer.Infrastructure` diretamente, para poder chamar `AddInfrastructure(...)` e registrar `DocsViewerDbContext` no DI — não há forma de configurar DI sem essa referência, já que Application (camada intermediária) não pode depender de EF Core/SqlServer sem violar "Application orquestra casos de uso" / "Infrastructure implementa persistência". A própria `docs/ARCHITECTURE.md` já diz, na seção Princípios, que "Web contém UI, endpoints e **composição**", o que textualmente já sugere esse papel de composition root. A referência foi adicionada para viabilizar a tarefa. Isso deveria ser formalizado explicitamente na lista de dependências de `docs/ARCHITECTURE.md` (edição simples) ou registrado como ADR (mudança arquitetural)?
**Impacto:** Afeta a leitura do grafo de dependências documentado; qualquer tarefa futura que também precise que Web componha Infrastructure já tem o precedente, mas o documento formal ainda não reflete essa aresta.
**Opções identificadas (sem escolher):** (1) atualizar apenas a lista de dependências em `docs/ARCHITECTURE.md` para incluir `Web -> Infrastructure`, por ser consequência direta do princípio de composição já aprovado; (2) exigir um ADR formal para essa aresta, tratando-a como decisão arquitetural nova; (3) manter como está e tratar a referência como detalhe de implementação não documentado no grafo.
**Status:** Aberta — não bloqueou a DV2-DEV-003 (a referência foi adicionada, pois sem ela a tarefa de configurar DI seria impossível de cumprir), mas fica registrada para confirmação/formalização.
